    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse">
            <ul class="nav navbar-nav navbar-right">
                ...
                <li class="active"><a href="${pageContext.request.contextPath}/my?index=1&number=2">我的资源</a></li>
...
<div class="container">
    ...
    <table class="table table-striped table-hover">
        ...
        <c:forEach items="${requestScope.MY_INFO.imgs}" var="img" varStatus="vs">
            <tr>
                <td>${vs.index+1+(MY_INFO.thisPage-1)*2}</td>
                ...
                <td>${img.downloads}</td>
                <td>
                    <a href="${pageContext.request.contextPath}/toupdate?id=${img.id}">修改</a>
                    <a href="${pageContext.request.contextPath}/delete?id=${img.id}">删除</a>
                </td>
            </tr>
        </c:forEach>
    ...
    <nav class="text-center">
        <ul class="pagination">
            <c:if test="${requestScope.MY_INFO.thisPage == 1}">
                <li><a href="javascript:;" title="上一页">&lt;</a></li>
            </c:if>
            <c:if test="${requestScope.MY_INFO.thisPage != 1}">
                <li>
                    <a href="${pageContext.request.contextPath}/my?number=2&index=${requestScope.MY_INFO.thisPage-1}"
                       title="上一页">&lt;</a></li>
            </c:if>
            <c:forEach begin="1" end="${requestScope.MY_INFO.pageNumber}" varStatus="vs">
                <c:if test="${vs.index == requestScope.MY_INFO.thisPage}">
                    <li class="active"><a href="javascript:;">${vs.index}</a></li>
                </c:if>
                <c:if test="${vs.index != requestScope.MY_INFO.thisPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/my?number=2&index=${vs.index}">${vs.index}</a>
                    </li>
                </c:if>
            </c:forEach>
            <c:if test="${requestScope.MY_INFO.thisPage == requestScope.MY_INFO.pageNumber}">
